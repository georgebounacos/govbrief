...
// Use [govbrief_share] for post sharing, [govbrief_share site="true"] for site sharing
function govbrief_share_shortcode($atts) {
    $atts = shortcode_atts(array('id' => null, 'site' => false), $atts);

    // If site="true", share the homepage instead of current post
    if (filter_var($atts['site'], FILTER_VALIDATE_BOOLEAN)) {
        return govbrief_site_share_links();
    }
...
// Site-level share links (for homepage)
function govbrief_site_share_links() {
