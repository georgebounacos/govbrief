<?php
/*
Template Name: Social Brief Generator (Instagram)
*/

// Password protection check 
if ( post_password_required() ) {
    get_header();
    echo '<div style="max-width: 800px; margin: 40px auto; padding: 36px;">';
...
// Height-based page break logic FOR INSTAGRAM (1080px wide, 4:5 ratio)
// Estimate: Header ~320px, Subscribe ~70px, Category title ~90px, Story ~100px each
$estimated_height = 390; // Start with header + subscribe
$page_break_height = 2200; // Higher limit for Instagram - they handle tall images well
$page_number = 1;
$total_stories = 0;
...
foreach($categories as $category) {
    ...
    if($posts && !empty($posts)) {
        ...
        if(!empty($category_stories)) {
            ...
            // Add category title height estimate (larger for Instagram)
            $estimated_height += 90; // Category title ~90px
            ...
            foreach($category_stories as $post) {
                // Check if we need a page break BEFORE adding this story
                if($estimated_height > $page_break_height && $page_number < 3) {
                    echo '</div>'; // Close current category-section
                    echo '<div class="page-break">✂️ --- PAGE BREAK ' . $page_number . ' --- Split here for Page ' . ($page_number + 1) . ' ✂️</div>';
                    $page_number++;
                    $estimated_height = 180; // Reset height counter for new page (buffer for margins, scaled for IG)
                    
                    // Re-open category section and title for new page
                    echo '<div class="category-section">';
                    echo '<div class="category-title" style="background: ' . $cat_color . ';">' . strtoupper($category) . '</div>';
                    $estimated_height += 90; // Add category title height
                }
                ...
                // Add story height estimate (~100px per story for Instagram)
...
<div class="links-section export-buttons">
    ...
    <p>Click the button below to download the brief as a PNG image optimized for Instagram (1080px wide, 4:5 ratio). If your brief is long, it will include page break markers showing where to split it into 2-3 separate images.</p>
    <button class="copy-button" onclick="downloadAsImage()">📥 Download as Image (PNG)</button>
    <p style="margin-top: 15px; font-size: 23px; color: #666;">Total: <?php echo $total_stories; ?> stories. Optimized for Instagram at 1080x1350px max per image.</p>
</div>
...
function downloadAsImage() {
    ...
    loadingMsg.style.cssText = 'position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(0,0,0,0.8); color: white; padding: 36px; border-radius: 10px; z-index: 9999; font-size: 18px;';
    ...
    html2canvas(exportContent, {
        ...
        canvas.toBlob(function(blob) {
            ...
            alert('Image downloaded! Open it in any image editor and split at the ✂️ page break markers (if any) to create your social media images.');
        }, 'image/png');
    }).catch(error => {
